...
namespace NRKernal
{
    ...
    public class NRSessionConfig : ScriptableObject
    {
        ...
        {
            // NRDebugger.Info("SYSTEM DEVICES: {0}", device);  
            // NRDebugger.Info("SECOND TARGET DEVICES: {0}", ProjectConfig.targetDeviceTypes); 
            return ProjectConfig ? ProjectConfig.targetDeviceTypes.Contains(device) : false;
        }

        public string GetTargetDeviceTypesDesc()
        {
            // NRDebugger.Info("FIRST TARGET DEVICES: {0}", ProjectConfig.GetTargetDeviceTypesDesc()); 
